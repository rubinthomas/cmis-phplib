...
/* Utility functions */
  ...
  function getLink($objectId, $linkName) {
    if (is_array($this->_link_cache) && array_key_exists($objectId, $this->_link_cache)) {
      if(is_array($this->_link_cache[$objectId]) && array_key_exists($linkName, $this->_link_cache[$objectId]))
...
<?php
  ...
  function query($q,$options=array()) {
    ...
    #echo print_r($ret->content_sent,1);
    #echo $ret->body;
    $objs = $this->extractObjectFeed($ret->body);
    $this->cacheFeedInfo($objs);
    return $objs;
...
<?php
   ...
   * @since CMIS-1.0
   */
  function appendContentStream($objectId, $content, $content_type, $options = array ()) {
  ...
  function addObjectToFolder($objectId, $targetFolderId, $options = array ()) {
    return $this->postObject($targetFolderId, $this->getTitle($objectId), $this->getObjectType($objectId), array (
      "cmis:objectId" => $objectId
    ), null, null, $options);
  }
